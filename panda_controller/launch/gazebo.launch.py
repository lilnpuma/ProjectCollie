...
def generate_launch_description():
    ...
    # Spawn camera
    camera_urdf = xacro.process_file(
        os.path.join(panda_world_path, "urdf", "camera.urdf.xacro"),
        mapping={"name": "TableCamera"},
    ).toxml()
    camera_urdf = camera_urdf.replace('"', '\\"')
    camera_urdf = '{name: \"TableCamera\", xml: \"' + camera_urdf +\
        '\", initial_pose: {position: {x: '+str(0.0)+',\
             y: '+str(-0.5)+', z: 2.75},\
    ...
    static_tf = Node(
        ...
        # parameters=[{'x': 0.0, 'y': -0.5, 'z': 2.75,
        # 'r': 0.0, 'p': 3.141/2, 'y': 3.141/2, 'frame_id':
        # 'world', 'child_frame_id': 'camera'}]
    )

    spawner_node = block_spawner_nodes(5)
    spawner_node = TimerAction(
        period=5.0,
        actions=spawner_node + [camera_node],
        condition=IfCondition(block_gen),
    )
...
def block_spawner_nodes(num_blocks):
    ...
    base_path = os.path.join(
        get_package_share_directory(
            "panda_world"), "urdf", "unit_box.urdf.xacro"
    )
    ...
    for block in blocks:
        ...
        block_urdf = xacro.process_file(
            base_path, mappings={"color": str(
                block), "block_id": "block_" + block}
        ).toxml()
        ...
        for i in range(0, num_blocks):
            spawn_args = (
                ...
                + str(random.uniform(x_range[0], x_range[1]))
                + ", y: "
                + str(random.uniform(y_range[0], y_range[1]))
                + ", z: 1.25},\
